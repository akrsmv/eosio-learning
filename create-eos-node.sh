...
sudo docker run --name eosio --publish 7777:7777 --publish 127.0.0.1:5555:5555 --volume /home/ubuntu/eos/contracts:/home/ubuntu/eos/contracts --detach eosio/eos:v1.4.2 /bin/bash -c "keosd --http-server-address=0.0.0.0:5555 & exec nodeos -e -p eosio --plugin eosio::producer_plugin --plugin eosio::chain_api_plugin --plugin eosio::history_plugin --plugin eosio::history_api_plugin --plugin eosio::http_plugin -d /mnt/dev/data --config-dir /mnt/dev/config --http-server-address=0.0.0.0:7777 --access-control-allow-origin=* --contracts-console --http-validate-host=false --filter-on='*'"
...
export WALLET_PASS=`cat TMP_WALLET_CREATE_OUTPUT | grep -o '"[a-zA-Z0-9]*"' | grep -o -E  [a-zA-Z0-9] | paste -sd ''`
echo $WALLET_PASS > ./eosio-default-wallet-password
cleos wallet open
cleos wallet unlock --password $WALLET_PASS
echo THIS WALLET PWD IS $WALLET_PASS and is stored in file eosio-default-wallet-password
